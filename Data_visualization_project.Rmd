...
To get rid of the high difference in total numbers between the two states we now look at the relative share in percent of stops to state population.
...
I chose a choropleth map again, to get a overview over all counties at once.
...
Up until now we looked at the overall amounts of smuggled goods over the years. What I am now interested in, is to see whether the amount of found contraband changes over the years we look at.
...
p2 <- ca_tx %>%
  ...
  geom_line(stat = "identity", size = 0.75)+
  state_colour_scale +
  labs(x = "Years", y = "Total Number of Stops", title = "Development in the Number of Traffic Stops per State over the Years")
...
We see a very clear pattern here for California. In may and in july the number of drunk drivers increases. This may be because of the holidays that take place in these months. For Texas we do not see such a strong pattern. As Texas is a very conservative state there may aren't that many drunk people as in California.
...
We can know obsereve similar patterns for both states. During the weekend the total number of drunk drivers increases. As most people don't work and go out in the evening on weekends, this is the pattern I expected to see. 
...
We see the same patterns as in the plot before. The only thing that changed, is that the difference between both states decreased for stops during the week. 
